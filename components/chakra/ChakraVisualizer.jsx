...
export default function ChakraVisualizer({ data }) {
  const [activeId, setActiveId] = useState(data?.chakras?.[0]?.id || 'root');
    ...
    <section className="relative grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
      <div className="relative flex min-h-[560px] flex-col items-center justify-between overflow-hidden rounded-[32px] border border-white/10 bg-black/70 p-6">
        ...
        <div className="relative z-10 flex flex-col items-center gap-6">
          ...
          <div className="relative h-[320px] w-[220px] rounded-full border border-white/10 bg-black/60">
          ...
          {data.chakras.map((chakra) => {
            ...
            return (
              <button
                ...
                className="group relative h-11 w-11 rounded-full border border-white/10 bg-black/70 transition"
                style={{
                  boxShadow: isActive ? `0 0 28px ${chakra.color}66` : 'none',
      ...
      <div className="flex flex-col gap-5">
        <div className="rounded-[28px] border border-white/10 bg-black/60 p-6 text-white shadow-[0_40px_80px_-50px_rgba(0,0,0,0.8)]">
          ...
          <div className="mt-5 grid gap-4 text-sm text-white/70">
            ...
            <p>
              <span className="text-white/50">Themes:</span> {active.themes.join(', ')}
            </p>
            <p>
              <span className="text-white/50">When off balance:</span> {active.imbalances.join(', ')}
            </p>
            <p>
              <span className="text-white/50">Gentle practices:</span> {active.practices.join(', ')}
            </p>
